...
(xL, xR)     = (-0.5, 0.5)
(yB, yL, yR) = (-0.5, 1.5, 2.0)

xlabels = (-3.5, -3, -2.5, -2, -1.5, -1, -0.5, 0, 0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4.0, 4.5)
ylabels = (-1.5, -1, -0.5, 0, 0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4.0, 4.5)
...
(startq1, startq2, startq3, startq4) = (0, 0, 0, 0)
(goalq1,  goalq2,  goalq3, goalq4)  = (pi/2, 0, 0, 0)

Dx = 0.1 #dmin = 0.1m in the lecture video
Dq = Dx/4.0 #(dmin/length of arm = dmin/4m)
...
if len(sys.argv) > 1:
    ...
    if grid == "grid1":
        ...
        # Define the start/goal states (joint values)
        (startq1, startq2, startq3, startq4) = (0, 0, 0, 0)
        (goalq1,  goalq2,  goalq3, goalq4)  = (pi/4, pi/3, 0, pi/3)
        RUN_TEST = True
        DRAW = False
...
class Visualization:
    ...
    def drawTip(self, node, *args, **kwargs):
        plt.arrow(node.xC, node.yC, 0.9*(node.xD-node.xC), 0.9*(node.yD-node.yC),
    ...
    def drawRobot(self, node, *args, **kwargs):
        ...
        plt.plot((node.xB, node.xC), (node.yB, node.yC), *args, **kwargs)
        ...
        plt.gca().add_artist(plt.Circle((node.xC,node.yC),*args,**kwargs))

    def drawNode(self, node, *args, **kwargs):
        self.drawTip(node, *args, **kwargs)

    def drawEdge(self, n1, n2, *args, **kwargs):
        return plt.plot(((n1.xC + n1.xD)/2, (n2.xC + n2.xD)/2),
                 ((n1.yC + n1.yD)/2, (n2.yC + n2.yD)/2), *args, **kwargs)
...
class Node():
    def __init__(self, q1, q2, q3, q4):
        ...
        (self.xD, self.yD) = (self.xC + cos(q1+q2+q3+q4), self.yC + sin(q1+q2+q3+q4))
        self.links = LineString([[0,0], [self.xA,self.yA],
                                 [self.xB,self.yB], [self.xC, self.yC], [self.xD, self.yD]])

    ############
    # Utilities:
    # In case we want to print the node.
    def __repr__(self):
        return ("<Joints %6.1fdeg,%6.1fdeg,%6.1fdeg,%6.1fdeg>" %
                (self.q1 * 180/pi, self.q2 * 180/pi, self.q3 * 180/pi, self.q4 * 180/pi))
    ...
    def intermediate(self, other, alpha):
        ...
        return Node(self.q1 + alpha *  (other.q1 - self.q1),
                    ...
                    self.q4 + alpha *  (other.q4 - self.q4))
    ...
    def distance(self, other):
        ...
        return sqrt((self.q1 - other.q1)**2 + (self.q2 - other.q2)**2 +
                    (self.q3 - other.q3)**2 + (self.q4 - other.q4)**2)
...
def rrt(startnode, goalnode, visual):
    ...
    while True:
        ...
        if val == 0:
            ...
            q4_coord = random.uniform(-pi, pi)
            targetnode = Node(q1_coord, q2_coord, q3_coord, q4_coord)
...
def main():
    ...
    startnode = Node(startq1, startq2, startq3, startq4)
    goalnode  = Node(goalq1,  goalq2,  goalq3, goalq4)
...
def run_test():
    ...
    filename = "{}_{}_four_links.csv".format(rrt_type, grid)
