...
#   RRT Functions
#
def rrt(startnode, goalnode, visual, t0):
...
def rrt_march(startnode, goalnode, visual, t0):
    # Start the tree with the startnode (set no parent just in case).
    startnode.parent = None
    tree = [startnode]
    def addtotree(oldnode, newnode):
...
def rrt_var(startnode, goalnode, visual, t0):
    # Start the tree with the startnode (set no parent just in case).
    startnode.parent = None
    tree = [startnode]
    def addtotree(oldnode, newnode):
...
def rrt_star(startnode, goalnode, visual, t0):
    # Start the tree with the startnode (set no parent just in case).
    startnode.parent = None
    tree = [startnode]
...
def rrt_bi(startnode, goalnode, visual, t0):
    # Start the tree with the startnode (set no parent just in case).
    startnode.parent = None
...
def run_test():
    ...
    while succesful_runs < N:
        ...
        if rrt_type == "rrt_reg":
            start = time.time()
            path, steps, num_nodes = rrt(startnode, goalnode, visual, time.time())
            dt = time.time() - start
        elif rrt_type == "rrt_march":
            start = time.time()
            path, steps, num_nodes = rrt_march(startnode, goalnode, visual, time.time())
            dt = time.time() - start
        elif rrt_type == "rrt_var":
            start = time.time()
            path, steps, num_nodes = rrt_var(startnode, goalnode, visual, time.time())
            dt = time.time() - start
        elif rrt_type ==  "rrt_star":
            start = time.time()
            path, steps, num_nodes = rrt_star(startnode, goalnode, visual, time.time())
            dt = time.time() - start
        elif rrt_type == "rrt_bi":
            start = time.time()
            path, steps, num_nodes = rrt_bi(startnode, goalnode, visual, time.time())
            dt = time.time() - start
        else:
            continue
...
if __name__== "__main__":
    if RUN_TEST:
        #DRAW = TRUE
        TIMEOUT = 5.0 #do not run any iteration for more than Timeout seconds
        run_test()
    else:
        main()
